...
def get_moves_and_stacks():
    ...
    for line in lines:
        ...
        if line.isspace():
            break
        for crate, i in enumerate(range(1, len(line), 4)):
            if not line[i].isspace():
                stacks[crate].insert(0, line[i])
    moves = [line.strip() for line in lines[line_num + 1 :] if not line.isspace()]
    return moves, stacks


# part 1
moves, stacks = get_moves_and_stacks()
for move in moves:
...
for move in moves:
    match = re.match(r"move (\d+) from (\d+) to (\d+)", move)
    num_moved, start_stack, end_stack = map(int, match.groups())
    crates_to_move = stacks[start_stack - 1][-num_moved:]
    del stacks[start_stack - 1][-num_moved:]
    stacks[end_stack - 1].extend(crates_to_move)
print("".join(stack[-1] for stack in stacks))
